...
export function Header({ onMenuClick }: { onMenuClick?: () => void }) {
  const { resetDay, endDay } = useStore();
  ...
  return (
    <header className="h-20 border-b border-border bg-[rgba(11,11,15,0.8)] backdrop-blur-md sticky top-0 z-10 flex items-center justify-between px-4 md:px-6 lg:px-8">
      ...
      <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
        <button 
          onClick={resetDay}
          className="flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 rounded-lg text-sm font-medium text-text-muted hover:text-text-main hover:bg-surface transition-colors"
        >
        ...
        <button 
          onClick={endDay}
          className="flex items-center gap-2 px-4 py-2 md:px-5 lg:px-6 md:py-2.5 rounded-lg text-sm font-medium bg-primary text-background hover:bg-primary-hover transition-all shadow-[0_0_15px_rgba(212,175,55,0.3)] hover:shadow-[0_0_25px_rgba(212,175,55,0.5)]"
        >
          <CheckCircle2 className="w-4 h-4" />
          <span className="hidden lg:inline">End Day & Score</span>
          <span className="lg:hidden">End Day</span>
        </button>
      </div>
    </header>
  );
